...
function updateStickyOffsets(){
  ...
  const cardHeads = [...document.querySelectorAll(".card:not(.search-card) > .cardhead")];
  const maxCardHeadH = cardHeads.length
    ? Math.ceil(Math.max(...cardHeads.map(h => h.getBoundingClientRect().height))) + 12
    : 74;

  document.documentElement.style.setProperty("--tableActionHeaderH", `${maxCardHeadH}px`);
}

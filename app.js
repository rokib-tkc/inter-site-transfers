...
(function initLenis(){
  ...
  const lenis = new Lenis({
    // Original site scrolling configuration — intentionally unchanged.
    duration: 1.75,
    lerp: 0.075,
...
const sentCard = document.getElementById("sentCard");
  ...
  const text = currentSheet === "PKY_TO_DTN" ? "Parkway to Denton" : "Denton to Parkway";
  const el = document.getElementById("directionLabel");
  const compactEl = document.getElementById("compactDirectionLabel");
  if (el) el.textContent = text;
  if (compactEl) compactEl.textContent = text;
}

function initialiseStickyLayout(){
  const appHeader = document.querySelector("body > header");
  const compactBar = document.getElementById("compactWorkflowBar");
  const workflowPanel = document.querySelector(".workflow-panel");
  const searchCard = document.getElementById("searchCard");

  function updateMeasurements(){
    const root = document.documentElement;
    const headerH = appHeader ? Math.ceil(appHeader.getBoundingClientRect().height) : 72;
    const compactH = compactBar ? Math.max(34, Math.ceil(compactBar.scrollHeight || 34)) : 34;
    const searchH = searchCard ? Math.ceil(searchCard.getBoundingClientRect().height) : 76;
    root.style.setProperty("--appHeaderH", `${headerH}px`);
    root.style.setProperty("--compactWorkflowH", `${compactH}px`);
    root.style.setProperty("--compactWorkflowOffset", compactBar?.classList.contains("is-visible") ? `${compactH}px` : "0px");
    root.style.setProperty("--searchCardH", `${searchH}px`);
  }
  ...
  if (window.ResizeObserver){
    ...
    [appHeader, compactBar, searchCard].filter(Boolean).forEach(el => ro.observe(el));
  }

  if (workflowPanel && compactBar && window.IntersectionObserver){
    const observer = new IntersectionObserver(([entry]) => {
      compactBar.classList.toggle("is-visible", !entry.isIntersecting && entry.boundingClientRect.top < 0);
      updateMeasurements();
    }, { rootMargin: `-${Math.ceil((appHeader?.getBoundingClientRect().height || 72))}px 0px 0px 0px`, threshold: 0 });
    observer.observe(workflowPanel);
  }
}

document.addEventListener("DOMContentLoaded", initialiseStickyLayout);
